...
public class Casinha : MonoBehaviour
{
    ...
    private bool casinhaVasculhada = false; // Impede o pop-up após vasculhar
    ...
    private void Update()
    {
        ...
        if (jogadorProximo && !casinhaVasculhada && !popUpAtivado && !inputCooldown)
        {
        ...
        if (jogadorProximo && casinhaVasculhada && !inputCooldown)
        {
            if (Input.GetKeyDown(KeyCode.E))
            {
    ...
    {
        popUpAtivado = true;
        popUpObject.TriggerPopUp("Vasculhar a casinha do Rex?",
            () => VasculharCasinha(), // Ação do "Sim"
            () => RecusarVasculhar()  // Ação do "Não"
        );
    }

    private void VasculharCasinha()
    {
        playerInventory.AddItem(recompensa, quantidadeRecompensa);
        casinhaVasculhada = true;
