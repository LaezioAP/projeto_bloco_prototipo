...
public class Lixeira : MonoBehaviour
{
    ...
    private bool lixeiraVasculhada = false; // Impede o pop-up após vasculhar
    ...
    private void Update()
    {
        ...
        if (jogadorProximo && !lixeiraVasculhada && !popUpAtivado && !inputCooldown)
        {
        ...
        if (jogadorProximo && lixeiraVasculhada && !inputCooldown)
        {
            if (Input.GetKeyDown(KeyCode.E))
            {
    ...
    {
        popUpAtivado = true;
        popUpObject.TriggerPopUp("Vasculhar a lixeira?",
            () => VasculharLixeira(), // Ação do "Sim"
            () => RecusarVasculhar()  // Ação do "Não"
        );
    }

    private void VasculharLixeira()
    {
        playerInventory.AddItem(recompensa, quantidadeRecompensa);
        lixeiraVasculhada = true;
