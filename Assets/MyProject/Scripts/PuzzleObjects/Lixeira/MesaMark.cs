...
public class MesaMark : MonoBehaviour
{
    ...
    private bool mesaVasculhada = false; // Impede o pop-up após vasculhar
    ...
    private void Update()
    {
        ...
        if (jogadorProximo && !mesaVasculhada && !popUpAtivado && !inputCooldown)
        {
        ...
        if (jogadorProximo && mesaVasculhada && !inputCooldown)
        {
            if (Input.GetKeyDown(KeyCode.E))
            {
    ...
    {
        popUpAtivado = true;
        popUpObject.TriggerPopUp("Bagunçar a mesa do Mark?",
            () => VasculharMesa(), // Ação do "Sim"
            () => RecusarVasculhar()  // Ação do "Não"
        );
    }

    private void VasculharMesa()
    {
